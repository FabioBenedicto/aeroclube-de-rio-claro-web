# CNAB History — Web Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> **Execute AFTER** `aeroclube-api/docs/superpowers/plans/2026-06-02-cnab-history-api.md` — este plano depende dos novos endpoints da API.

**Goal:** Redesenhar a página CNAB com duas tabelas (remessas e retornos) e modal de seleção de faturas para gerar remessa.

**Architecture:** A página `Cnab.tsx` é reescrita com dois blocos independentes. Um modal extrai a seleção de faturas que hoje existe inline. `src/api/cnab.ts` é atualizado para os novos endpoints. `src/types/index.ts` recebe dois novos tipos.

**Tech Stack:** React 19, TypeScript, Tailwind CSS, React Query, Lucide React, componentes `Badge`, `Checkbox`, `Pagination`, `DateInput` existentes.
...
| Arquivo | Ação |
|---------|------|
| `src/types/index.ts` | Adicionar `CnabRemessa` e `CnabRetorno` |
| `src/api/cnab.ts` | Atualizar `generateRemessa` + adicionar `getRemessas`, `downloadRemessa`, `getRetornos` |
| `src/pages/cnab/Cnab.tsx` | Reescrever completamente |

---

## Task 1: Tipos + funções de API

**Files:**
- Modify: `src/types/index.ts`
- Modify: `src/api/cnab.ts`
...
- [ ] **Step 2: Atualizar `src/api/cnab.ts`**
...
```bash
git add src/types/index.ts src/api/cnab.ts
git commit -m "feat: add CnabRemessa/CnabRetorno types and update API functions"
```

---

## Task 2: Redesenho de Cnab.tsx

**Files:**
- Modify: `src/pages/cnab/Cnab.tsx`

- [ ] **Step 1: Substituir o conteúdo completo de `src/pages/cnab/Cnab.tsx`**

```tsx
import React, { useState, useRef } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { Download, Upload, X, ChevronRight, ChevronLeft, Plus } from 'lucide-react';
import {
  getBillsPending,
  generateRemessa,
  downloadRemessa,
          ...
          <table className="w-full border-collapse text-[13px]">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="px-3.5 py-8 text-center text-ink-3">Nenhuma fatura em aberto</td></tr>
              ) : bills.map(bill => (
                <tr
                  key={bill.id}
            ...
            {retornosLoading ? (
              ...
              <React.Fragment key={r.id}>
                <tr className="border-b border-line hover:bg-bg-hover">
                  ...
                  <td className={tdCls}>
                    <Badge variant="success">{r.paid_count}</Badge>
                  </td>
                  <td className={tdCls}>
                    {r.rejected_count > 0
...
git commit -m "feat: redesign CNAB page with remessa/retorno history tables and bill selection modal"
```
