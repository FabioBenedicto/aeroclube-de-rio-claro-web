      ...
      {steps.map((label, i) => {
        ...
        return (
          <span key={n} className="flex items-center gap-1.5">
            <span className={`w-5 h-5 rounded-full text-[11px] font-semibold flex items-center justify-center ${step === n ? 'bg-accent text-white' : 'bg-bg-sunk text-ink-3'}`}>
              {n}
            </span>
    ...
    return (
      <Modal onClose={onClose} maxWidth={520}>
        <Modal.Header title="Nova fatura" onClose={onClose}>
          <StepIndicator step={1} steps={['Cliente', 'Títulos']} />
        </Modal.Header>
        <Modal.Body>
        ...
        <>
          <span className="text-[13px] text-ink-3">
            {Object.keys(selected).length > 0
              ? `${Object.keys(selected).length} título(s) · R$ ${formatBRL(total)}`
              : 'Nenhum selecionado'}
          </span>
          <div className="flex gap-2">
            ...
            <Button
              variant="primary"
              disabled={Object.keys(selected).length === 0 || total <= 0}
              onClick={() => onSave({
