...
export function NewPayableModal({ customers, students, instructors, partners, employees, planes, companies, initialPlaneId, onClose, onSave }: {
    ...
    <Modal onClose={onClose} maxWidth={480}>
      <div className="flex items-start justify-between px-5 py-4 border-b border-line gap-3">
        <div className="flex items-center gap-3">
          <span className="text-[15px] font-semibold text-ink">Novo título a pagar</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1.5"><label className="text-[12px] font-medium text-ink-2">Valor</label>
                <div className="flex rounded-md border border-line overflow-hidden focus-within:border-accent focus-within:shadow-[0_0_0_3px_var(--focus)]">
                  ...
                  <input inputMode="numeric" placeholder="0,00" className="flex-1 px-2.5 py-[7px] border-0 bg-bg-elev text-ink text-[13px] font-mono outline-none" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: maskCurrency(e.target.value) }))} />
                </div>
              </div>
            </div>
          </>
        ...
        <div className="flex items-center gap-2">
          ...
          {step < totalSteps
            ...
            : <Button variant="primary" onClick={() => onSave({
              ...
              total_amount: parseCurrency(form.amount),
              expiration_date: form.due_date ? new Date(form.due_date).toISOString() : undefined,
              payable_type_id: form.product ? Number(form.product) : undefined,
