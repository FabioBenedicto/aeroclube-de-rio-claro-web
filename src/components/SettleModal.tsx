...
export default function SettleModal({ rec, creditBalance = 0, onClose, onSave }: Props) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-bg-elev border border-line rounded-[10px] w-full max-w-[480px] shadow-[var(--shadow)] flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-[18px] overflow-y-auto flex-1 flex flex-col gap-4">
          <div className="flex items-start gap-4 px-3.5 py-3 bg-bg-sunk rounded-md">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-ink-3 mb-0.5">Título</div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex rounded-md border border-line overflow-hidden focus-within:border-accent focus-within:shadow-[0_0_0_3px_var(--focus)]">
                    ...
                    <input
                      ...
                      type="number"
                      step="0.01"
                      value={useCredit ? String(cashNeeded) : (mode === 'total' ? String(remaining) : amount)}
                      onChange={e => setAmount(e.target.value)}
                      disabled={mode === 'total' || useCredit}
                    />
                  </div>
                </div>
                <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center justify-end gap-2 px-[18px] py-3.5 border-t border-line">
          <span className="text-[11.5px] text-ink-3 mr-auto">
            Saldo restante: <strong className="font-mono">R$ {formatBRL(Math.max(0, remaining - totalEffective))}</strong>
          </span>
          <button className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-[13px] font-medium border border-line bg-bg-elev text-ink-2 cursor-pointer hover:bg-bg-hover hover:text-ink" onClick={onClose}>
            Cancelar
          </button>
          <button className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-[13px] font-medium bg-accent border border-accent text-white cursor-pointer hover:opacity-90" onClick={handleSubmit}>
            <CheckIcon size={14} /> Confirmar recebimento
          </button>
        </div>
      </div>
    </div>
